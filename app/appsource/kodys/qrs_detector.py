...
class QRSDetectorOffline(object):
    ...
    def __init__(self, ecg_data=None,ecg_data_path=None, verbose=True, log_data=False, plot_data=False, show_plot=False):
        """
        ...
        self.signal_frequency = 250.0  # Set ECG device frequency in samples per second here.

        self.filter_lowcut = 5.0
        self.filter_highcut = 15.0
        self.filter_order = 2

        self.integration_window = 30  # Increased for better smoothing at 250Hz.

        self.findpeaks_limit = 0.35   # Lowered to capture more potential peaks for classification.
        self.findpeaks_spacing = 50

        self.refractory_period = 50   # 200ms at 250Hz is 50 samples.
    ...
    def load_ecg_data(self):
        ...
        """
        if self.ecg_data:
            self.ecg_data_raw = np.asarray(self.ecg_data, dtype=np.float32)
        else:
    ...
    def print_detection_data(self):
        ...
        # print(self.qrs_peaks_indices, len(self.qrs_peaks_indices))
    ...
    def plot_detection_data(self, show_plot=False):
        ...
        # plot_data(axis=axarr[5], data=self.ecg_data_detected, title='Raw ECG measurements with QRS peaks marked (black)')
